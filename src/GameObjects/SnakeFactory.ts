import { CircleGeometry, Color, DoubleSide, Mesh, MeshBasicMaterial, MeshLambertMaterial, SphereGeometry } from "three";
...
export default class SnakeFactory {
    create(length: number, color: string): Snake {
        ...
        const sMaterial = new MeshBasicMaterial({color: color});
        ...
        const eyeMaterial = new MeshBasicMaterial({color: "#fff"});
        ...
        const pupilMaterial = new MeshBasicMaterial({color: "#000", "side": DoubleSide});
